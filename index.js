...
async function run() {
  try {
    ...
    const itemCollection2 = database.collection("artCategory")
    ...
    app.post('/artCategory', async (req, res) => {
      const item = req.body;
      // console.log(user, "from server")
      // const result = await itemCollection.insertOne(item);
    ...
    app.get('/artCategory-data/:subCategory', async (req, res) => {
      console.log(req.params.subCategory)
      const result = await itemCollection2.find({ sub_category: req.params.subCategory }).toArray();
      res.send(result)
    })
    ...
    app.delete('/artCategory/:id', async (req, res) => {
      const id = req.params.id;
      const query = { _id: new ObjectId(id) };
    ...
    app.put('/artCategory/:id', async (req, res) => {
      ...
      const result = await itemCollection2.updateOne(filter, updateDoc, options);
      res.send(result);
    })
